...
class double_filter:
    ...
    Если использовать этот фильтр, то значит анализируя полученные волны надо принимать во внимание, что факт формирования волны будет получен, полсе закрытия первой свечи после пика
    '''
    def __init__(self, wave_direction: direction) -> None:
        self.wave_direction = wave_direction
