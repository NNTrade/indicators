...
from .gain_loss import SmaGainBuilder, SmaLossBuilder
...
class RSBuilder:
    ...
    def get_name_for(self, sr: pd.Series)->str:
        return get_col_name(Indicators.RS, [str(self.__period)], [sr.name])

    def get_for(self, sr: pd.Series, is_last:pd.Series=None, indicators_df:pd.DataFrame = None) -> pd.Series:
...
class RSIBuilder:
    ...
    def get_name_for(self, sr: pd.Series)->str:
        return get_col_name(Indicators.RSI, [str(self.__period)], [sr.name])
    ...
    def _get_rs(self, sr: pd.Series, is_last:pd.Series=None, indicators_df:pd.DataFrame = None) -> pd.Series:
        rs_builder = RSBuilder(self.__period)
